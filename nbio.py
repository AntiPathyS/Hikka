        ...
        try:
            if norm_args[0] in args_restore:
                ...
                try:
                    ...
                    return await utils.answer(message, f"<emoji document_id=5188391205909569136>✅</emoji><b>Бекап зарлиста {a}загружен!</b>")
                except Exception as e:
...
#######################################################
        ...
        if re.fullmatch(r"д\s@\d{3,12}.{,10}", text, flags=re.ASCII):
            if str(sndr_id) != str(get_me.id):
                return            
            k = ''
            pas = 0
