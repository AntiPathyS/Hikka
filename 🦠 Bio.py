...
Ваша вторая рука в биовойнах)
    ...
    strings = {
        ...
        "not_reply": "<emoji document_id=5465665476971471368>❌</emoji> Нет реплая.",
        
        "not_args": "<emoji document_id=5465665476971471368>❌</emoji> Нет аргументов.",
        
        "nolink": "<emoji document_id=5465665476971471368>❌</emoji> Нет ссылки на жертву.",

        "hueta": "🤔 Что за хуета?",
        
        "r.save":   
            "<emoji document_id=5188391205909569136>✅</emoji> Жертва <b><code>{}</code></b> сохранена.\n"
            "<b>☣️ +{}{}</b> био-опыта.",
        "auto.save":   
            "<emoji document_id=5188391205909569136>✅</emoji> Жертва <b><code>{}</code></b> сохранена.\n"
            "<b>☣️ {}+{}</b> био-опыта.",        
        "search":
            "<emoji document_id=5188391205909569136>✅</emoji> Жертва <code>{}</code> приносит:\n"
            "<b>☣️ +{} био-опыта.</b>\n",
        
        "nf": "<emoji document_id=5465665476971471368>❌</emoji> Жертва не найдена.",
        
        "no_user": "<emoji document_id=5465665476971471368>❌</emoji> user {} don't exist.",

        "nous": "<emoji document_id=5465665476971471368>❌</emoji> Жертва или пользователь не существует.",
        ...
        "user_add": "<emoji document_id=5188391205909569136>✅</emoji> Саппорт <b><code>{}</code></b> добавлен!",
        
        "wrong_nick": "<b>📝 Введите ник.</b>",
        
        "nick_add": "🔰 Ник <b>{}</b> установлен!",
        
        "dov_start": "<b><emoji document_id=5188391205909569136>✅</emoji> Успешно запущено!</b>",
        
        "dov_stop": "<b>❎ Успешно остановлено.</b>",
        
        "dov.wrong_args": 
            "<b><emoji document_id=5465665476971471368>❌</emoji> Неизвестный аргумент.</b>\n"
        ...
        "wrong_ot-do": '<emoji document_id=5465665476971471368>❌</emoji> еблан, Используй <b>правильно</b> функцию "от-до".',
        
        "no_sargs": "<emoji document_id=5465665476971471368>❌</emoji> Не найдено совпадение в начале строк с аргументами.",
        
        "no_link": "<emoji document_id=5465665476971471368>❌</emoji> Ссылка не найдена.",
        
        "too_much_args": "<emoji document_id=5465665476971471368>❌</emoji> Кол-во аргументов <b>больше</b> одного, либо начинается <b>не</b> со знака <code>@</code>",
        
        "no_zar_reply": "<emoji document_id=5465665476971471368>❌</emoji> Нет реплая на сообщение ириса о заражении.",
        
        "empty_zar": "<emoji document_id=5465665476971471368>❌</emoji> Список заражений пуст.",
        
        "wrong_zar_reply": '<emoji document_id=5465665476971471368>❌</emoji> Реплай <b>не</b> на сообщение ириса о заражении "<b>...подверг заражению...</b>"',
        
        "wrong_cmd": "<emoji document_id=5465665476971471368>❌</emoji> Команда введена некорректно.",
        
        "empty_ex": "<emoji document_id=5465665476971471368>❌</emoji> Cписок исключений пуст.",
        
        "tids": "<b><emoji document_id=5188391205909569136>✅</emoji> Id'ы успешно извлечены.</b>",
        
        "tzar": "<emoji document_id=5188391205909569136>✅</emoji> Заражения завершены.",
        ...
        try:
            if norm_args[0] in args_restore:
                ...
                try:
                    ...
                    return await utils.answer(message, f"<emoji document_id=5456181450447527779>🌞</emoji> <b>Бекап зарлиста {a}загружен!</b>")
                except Exception as e:
...
#######################################################
        if self.config["Доступ к зарлисту"] == True:
            ...
            if re.search(r"з", text, flags=re.ASCII):
                ...
                if re.fullmatch(r".{,5}заразить [0-10] @\d{5,11}", reply.text.lower(), flags=re.ASCII):
                    zhertva = "@" + str(reply.text.split("@")[1])

                if re.fullmatch(r".{,5}заразить @\D\w{3,32}", reply.text.lower(), flags=re.ASCII):
                    user = "@" + str(reply.text.split("@")[1])
                    get_id = await message.client.get_entity(user)
                    get_id = get_id.id
                    zhertva = "@" + str(get_id)

                if re.fullmatch(r".{,5}заразить [0-10] @\D\w{3,32}", reply.text.lower(), flags=re.ASCII):
                    user = "@" + str(reply.text.split("@")[1])
                    get_id = await message.client.get_entity(user)
                    get_id = get_id.id
                    zhertva = "@" + str(get_id)
